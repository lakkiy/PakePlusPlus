            ...
            <template #footer>
                ...
                >
                    ...
                    <el-button
                        type="success"
                        plain
                        @click="checkReposAndGetShas(false)"
                    >
                        {{ t('sync') }}
                    </el-button>
                    <!-- user confirm -->
            ...
            if (userInfo.status === 200) {
                ...
                try {
                    if (userInfo.data.login !== upstreamUser) {
                        await checkReposAndGetShas(tips)
                    } else {
...
// check repositories and get commit shas
const checkReposAndGetShas = async (tips: boolean = true) => {
    testLoading.value = true
    // check if repositories exist (no auto-forking)
    const repoExists: any = await Promise.all([
        checkPakePlusRepos('PakePlus'),
        checkPakePlusRepos('PakePlus-iOS'),
        checkPakePlusRepos('PakePlus-Android'),
    ])
    
    console.log('Repository existence check:', repoExists)
    
    // sync all branch (only if repos exist)
    if (repoExists.some((exists: boolean) => exists)) {
...
// check if user repositories exist (removed auto-fork functionality)
const checkPakePlusRepos = async (repo: string = 'PakePlus') => {
    try {
        // Just check if the user already has the repo, no auto-forking
        ...
        if (res.status === 200) {
            ...
            console.log('Repository does not exist:', repo)
            return false
        }
    } catch (error) {
        console.log('Repository check failed:', repo, error)
        return false
    }
}
